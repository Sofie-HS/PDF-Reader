...
try:
    ...
    st.error(
        "The temporary workbook could not be reopened. Use Delete workbook "
        "and clear memory, then run a smaller batch."
    )
    st.exception(exc)
    st.stop()
...
workbook_download_bytes = read_workbook_for_download(result_path)

st.download_button(
    "Download Concrete Test Log (.xlsx)",
    data=workbook_download_bytes,
