...
def add_series_with_trendline(fig, data, x_field, y_field, name, color, symbol):
    ...
    if len(plot_data) >= 3 and plot_data[x_field].nunique() >= 3:
        x = plot_data[x_field].astype(float).to_numpy()
        y = plot_data[y_field].astype(float).to_numpy()
        coefficient_a, coefficient_b, intercept = np.polyfit(x, y, 2)
        x_line = np.linspace(x.min(), x.max(), 200)
        y_line = coefficient_a * x_line ** 2 + coefficient_b * x_line + intercept
        y_pred = coefficient_a * x ** 2 + coefficient_b * x + intercept
        ss_res = np.sum((y - y_pred) ** 2)
        ss_tot = np.sum((y - np.mean(y)) ** 2)
        r_squared = 1 - ss_res / ss_tot if ss_tot else np.nan
        equation = (
            f"y = {coefficient_a:.3f}x² + {coefficient_b:.2f}x + {intercept:.1f}"
        )
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=y_line,
                mode="lines",
                name=f"{name} quadratic fit ({equation}; {r_text})",
...
class NamedBytesIO(io.BytesIO):
    """An in-memory PDF buffer that keeps the original filename."""
...
def expand_uploaded_items(uploaded_items):
    """Return PDF buffers from direct PDF uploads or ZIP folder uploads."""
    pdf_items = []
    upload_notes = []
        ...
        if lower_name.endswith(".zip"):
            try:
                ...
                with zipfile.ZipFile(item) as archive:
                    ...
                    upload_notes.append(
                        f"{item.name}: found {len(pdf_names)} PDF file(s) in the ZIP folder."
                    )
            except zipfile.BadZipFile:
                upload_notes.append(f"{item.name}: could not read this ZIP file.")

    return pdf_items, upload_notes


st.title("Concrete Test Dashboard")
st.caption(
    "Turn a folder of concrete test reports into one downloadable Excel log and "
    "interactive strength-analysis charts."
)
...
with st.expander("How to use this tool", expanded=True):
    st.markdown(
        ...
        **Step 1: Prepare the reports**
        - Keep the original PDF names. The app uses the filename to read the report date and 4-digit Sample ID.
        - To upload a full folder, right-select the folder in Windows and create a `.zip` file. Do not rename the PDFs inside it.

        **Step 2: Upload the data**
        - Drop one ZIP folder, multiple ZIP folders, individual PDFs, or a mix into the upload area below.
        - The app opens ZIP files in memory and searches all nested folders for PDFs.

        **Step 3: Let the app extract the data**
        - The app reads Location Details, air temperature, concrete temperature, slump, air content, minimum temperature, maximum temperature, and 7-day/28-day strength averages.
        - A progress message shows how many reports have been processed.

        **Step 4: Review the master table**
        - Check the Concrete Test Log for missing or unexpected values.
        - Invalid filenames or unreadable reports appear under Processing details, not in the Excel workbook.

        **Step 5: Download and analyze**
        - Use the download button to save the formatted `.xlsx` file.
        - Review the charts below the table. Blue represents 7-day strength and orange represents 28-day strength.
        - Each chart uses a second-order polynomial curve, also called a quadratic best-fit curve.
        """
    )

st.subheader("Step 1: Upload PDF reports or a ZIP folder")
uploaded_files = st.file_uploader(
    "Drop a ZIP folder or concrete test PDFs here",
    type=["pdf", "zip"],
    accept_multiple_files=True,
    help=(
        "For a full folder, compress the folder into a ZIP file and upload the ZIP. "
        "The app also accepts multiple individual PDFs."
    ),
)
...
pdf_files, upload_notes = expand_uploaded_items(uploaded_files)
...
st.subheader("Step 2: Extract report information")
records = []
skipped = []
progress = st.progress(0, text="Opening reports and reading concrete-test data...")

for index, uploaded_file in enumerate(pdf_files, start=1):
    try:
    ...
    progress.progress(index / len(pdf_files), text=f"Processed {index} of {len(pdf_files)} PDFs")
...
st.divider()
st.header("Step 5: Explore the strength relationship charts")
st.caption(
    "Blue circles and blue second-order polynomial curves represent 7-day averages. "
    "Orange diamonds and orange second-order polynomial curves represent 28-day averages. "
    "Both axes use numbered numeric tick marks."
)
